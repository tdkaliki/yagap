...
workflow LR_RNASEQ_MAPPING {
    ...
    main:
        ...
        if (params.run_clean_minimap2) {
            MINIMAP2_LR_MAPPING(lr_rna_seq_reads, genome)
            ch_versions = ch_versions.mix(MINIMAP2_LR_MAPPING.out.versions)
            STRINGTIE_MINIMAP(MINIMAP2_LR_MAPPING.out.minimap2_bam)
            ch_versions = ch_versions.mix(STRINGTIE_MINIMAP.out.versions)
        }
        else{
            MINIMAP2_LR(lr_rna_seq_reads, genome)
            ch_versions = ch_versions.mix(MINIMAP2_LR.out.versions)
            SAM_TO_BAM(MINIMAP2_LR.out.minimap2_sam)
        ...
        stringtie_gtfs=STRINGTIE_MINIMAP.out.stringtie_gtf.map { sample_id, file_path -> file_path }.collect().map { file_paths -> ['lr_rnaseq',file_paths] }
